...
func update(hp: int, xp: int, xp_next: int, level: int, time: float):
	$Control/VBoxContainer/LabelXP.text   = " XP: " + str(xp) + "/" + str(xp_next)
	$Control/VBoxContainer/LabelLevel.text = " Nível: " + str(level)
	$Control/VBoxContainer/LabelTime.text  = " Tempo: " + str(int(time)) + "s"
